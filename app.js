...
class App extends FlowApp{
	...
	async initConfig(){
		...
		if(reset) {
			try {
				...
				alert('Error resetting Gor-Desktop data folder: '+ex);
			}
	...
	*/
	dataDirInitError(){
		console.log(`Please start app with --init=/path/to/data/dir or --init for default (~/.gor-desktop/data)`);
		this.exit();
	}
	...
	*/
	...
	async removeDataDir(){
		try {
			let datadir2path = path.join(this.dataFolder, "gord-kd0", "gor-mainnet", "datadir2");
	...
	*/
	...
	getDefaultConfig(){
		...
		Object.entries(config.modules).forEach(([k,v]) => {
			...
			if(['gord' ,'gorminer'].includes(type)) {
	...
	setSkipUTXOIndex(skipUTXOIndex){
		...
		return this.setModuleArgs("gord:", {}, {"skip-utxoindex":skipUTXOIndex})
	}
	getSkipUTXOIndex(){
		let {args, params} = this.getModuleArgs("gord:");
	...
	setUpnpIfMissing(upnpEnabled) {
		if (this.config?.modules) {
			Object.keys(this.config.modules).forEach((k) =>{
				if (k.startsWith('gord:') && !('upnpEnabled' in this.config.modules[k])) {

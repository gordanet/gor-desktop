...
class Build {
    constructor() {
        ...
        if(this.args.folder)
            ...
            this.FOLDER = path.join(os.homedir(),'.gor-desktop','build');
        ...
        this.BIN = path.join(os.homedir(),'.gor-desktop','bin',this.PLATFORM_ARCH);
    ...
    help() {
        const text = 
`Gor-Desktop builder ${pkg.version}
Usage: build <flags>
Where <flags> are:
...
`;
    ...
    async main() {
        try {
            ...
            this.log(`Gor-Desktop builder ${pkg.version}`);
    ...
    async build() {
        try {
            ...
            this.TOOLS = path.join(this.FOLDER,'tools');
            // this has migrated to ~/.gor-desktop/bin 
            ...
            const dest = path.join(this.GOSRC,'github.com/gordanet/');
            ...
            for(const repo of repos.gor) {
                this.log(`git clone git@github.com:gordanet/${repo}`);
                await this.clone(`git@github.com:gordanet/${repo}`, dest, {branch});
            }
            ...
            if(this.args['with-extras']) {
                ...
                for(const repo of repos.extras) {
                    this.log(`git clone git@github.com:gordanet/${repo}`);
                    await this.clone(`git@github.com:gordanet/${repo}`, dest);
                }
            }
	...
	download(url, file) {
        ...
        return new Promise((resolve, reject) => {
            ...
            progress(
                request({ url, headers: { 'User-Agent': 'Gor-Desktop' } }), 
...
Before building Gor, you need to do some setup:
From the gor-desktop folder:

...
module.exports = class Simulator extends Daemon {
	...
	async start() {
		...
		let defaults = {
			...
			// dbuser: 	'gor', // Database user
			// dbpass: 	'gor', // Database password
			// dbname: 	'GOR', // Database name
...
//		let {config:daemons} = await this.get("get-modules-config");
		...
		if(!args.miningaddr) {
			args.miningaddr = 'gortest:qpjmmd8fc20c6mf5zxr6qax0369g0aufyg6aqcel9u';
			//args.miningaddr = 'gor:qqg767gkznmsuah5xeer9f72lwqu642nqvlmujtpmk';
			//args.miningaddr = 'gor:qr35ennsep3hxfe7lnz5ee7j5jgmkjswsslwxj42ta';
		}		
		...
		//console.log("++++++++++++++++ GORMINER VSARGS")

		await this.VSARGS([this.getDeployableBinary('gorminer'), '--version']);

		args = Object.entries(args);

		const flags = utils.args();
		//console.log("++++++++++++++++ GORMINER FLAGS",flags)

		this.options = {
			args : () => {
				// this.log(`--addresslist=${configFile}`,`--cert=${rpcCertFile}`);
				let _args_ = [
					this.getBinary('gorminer'),
			...
			stdout : (data) => { 
				...
				// // console.log("gorminer match:", match);
		...
		//console.log("GORMINER STATUS:",this.manager.enableMining,flags.mine);
		if(this.manager.enableMining || flags.mine) {
			dpc(0, () => {
				//console.log("----------------- GORMINER STARTING")
				this.run()
			})
		} else {
			//console.log("----------------- GORMINER NOT RUNNING")
			this.state = 'disabled';
			dpc(0, () => {
				this.writeToSink('\r\n\r\nGorminer is disabled. Please use Enable Mining option to start it.\r\n\r\n');
			})
		}

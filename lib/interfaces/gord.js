...
const {Client:GorGRPC} = require('@gor/grpc-node');
const pkg = require("../../package");
const natUpnp = require("@runonflux/nat-upnp");

module.exports = class Gord extends Daemon {
	...
	start() {
		...
		return new Promise(async (resolve,reject) => {
			...
			if (!("uacomment" in args)) {
				const uacomment = `gor-desktop_${pkg['version']}`
				this.writeToSink(`GORD: Adding uacomment=${uacomment} to arguments\n`);
			...
			if (this.task.conf['upnpEnabled'] && args['listen'] && args['listen'].indexOf(":") !== -1) {
				...
				let externalip = await this.getExternalIp();
				if (externalip) {
					if (!("externalip" in args)) {
						this.writeToSink(`GORD: Adding externalip=${externalip} to arguments\n`);
						args['externalip'] = externalip;
					}
					const publicPort = await this.forwardPort(nodePort, this.upnpRetries);
					if (publicPort) {
						let externalip = args['externalip'].split(':')[0];
						this.writeToSink(`GORD: Changing externalip from ${args['externalip']} to ${externalip}:${publicPort} in arguments\n`);
						args['externalip'] = `${externalip}:${publicPort}`;
						if (publicPort !== nodePort) {
							this.writeToSink(`GORD: Changing listener port from ${nodePort} to ${publicPort} in arguments\n`);
							args['listen'] = `${args['listen'].split(':')[0]}:${publicPort}`;
						}
						if (!("maxinpeers" in args)) {
							this.writeToSink(`GORD: Adding maxinpeers=${this.upnpMaxInPeers} to arguments\n`);
			...
			const configFile = path.join(this.folder, 'gord.conf');
			...
			await this.VSARGS([this.getDeployableBinary('gord'),'--version']);

			this.run({
				args : async () => {
					let _args_ = [
 						this.getDeployableBinary('gord')
					];
			...
			resolve();
			// mutes all gord processes by default
			this.mute = true;
		})
	}
		...
		return new Promise(async (resolve, reject) => {
	        ...
	        try {
				...
				this.data = this.sourcesToData(sources);
	             ...
	             console.log('GORD monitor:'.bold,ex);
	            return reject(ex);
	        }

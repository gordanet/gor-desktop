...
class GorProcessManager extends EventEmitter{
	constructor(controller = null, dataFolder="", appFolder="") {
		...
		//console.log("this.apps XXXXXXXXX ", JSON.stringify(this.apps, null, "\t"))
		this.strings = { };
	...
	async start(daemons={}) {
		...
		const startupTypeOrder = ['gord','mqtt','mysql','pgsql','gpuminer','gorminer','txgen','app'];
		const typeMap = { };
	...
	async stop() {
		//const shutdownTypeOrder = ['gord','mqtt','mysql','pgsql' ,'txgen'];
		//const typeMap = { };
		let jobs = this.tasks.map((t)=>{
			//console.log("stop:t:", t.name, "imp:", JSON.stringify(t, null, "\t"))
			if(t.impl) {
	...
	getDeployableBinary(...args) {
		...
		switch(buildType) {
			...
			case 'local': {
				return path.join(os.homedir(),'.gor-desktop','bin',utils.platform, ...args)+this.PLATFORM_BINARY_EXTENSION;
			}			
		}
	}
	...
	enumerateApps() {
		...
		return packages.map(({ folder, pkg }) => {
			let { name, description, main, engines, gor-desktop } = pkg;
			return {
				folder, name, description, main, engines, ...gor-desktop
			}
		});
	}
	...
	registerString(owner, valueType, ident, value) {
		try {
			...
			if(!isNaN(port)) {
				let key1 = `${type}-PORT`.toUpperCase();
				let key2 = `${id}-PORT`.toUpperCase();
				if(ident && ident.toLowerCase() != type.toLowerCase()) {
					key1 = `${type}-${ident}-PORT`.toUpperCase();
					key2 = `${id}-${ident}-PORT`.toUpperCase();
				}

				// console.log("REGISTERING STRING:", key1, value,'owner:',owner);
				// console.log("REGISTERING STRING:", key2, value,'owner:',owner);

				this.strings[key1] = port;
				this.strings[key2] = port;
			} else {
	...
	startProcMon() {
		...
		this.procmon = new Daemon(this, {
			ident : 'procmon:pm0', key : 'procmon-pm0', type : 'procmon',
			id : 'pm0',
			hidden : true,
			name: 'procmon'
		});
	...
	setEnableMining(enableMining) {
		...
		this.tasks.forEach((task) => {
			if(/gpuminer/ig.test(task.type)) {
				let {impl} = task;
				if(!impl){
					console.log("###### miner missing: 1", task)
				}else{
					if(this.enableMining) {
						impl.run();
					}else{
						impl.silentTerminate();	
					}
				}
			}
		})
	...
	restartMining() {
		...
		this.tasks.forEach((task) => {
			if(/gpuminer/ig.test(task.type)) {
				let {impl} = task;
				if(!impl){
					console.log("###### miner missing: 2", task)
				}else{
					impl.silentTerminate();
				}
			}
		...
		dpc(500, ()=>{
			this.tasks.forEach((task) => {
				if(/gpuminer/ig.test(task.type)) {
					let {impl} = task;
					if(!impl){
						console.log("###### miner missing: 3", task)
					}else{
						impl.run();
					}
				}
...
module.exports = GorProcessManager;

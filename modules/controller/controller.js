...
const {RPC} = require('@gor/grpc-node');
...
//TODO
window.PWA_MODULES={};
window.PWA_MODULES["@gor/wallet-pwa"] = "N/A";
window.PWA_MODULES["@aspectron/flow-ux"] = "N/A";
window.PWA_MODULES["@gor/ux"] = "N/A";
window.PWA_MODULES["@gor/grpc-web"] = "N/A";
window.PWA_MODULES["@gor/wallet"] = "N/A";
window.PWA_MODULES["@gor/grpc"] = "N/A";
window.PWA_MODULES["@gor/core-lib"] = "N/A";

class Gor-DesktopApp extends FlowApp{
	render(){
		let walletMeta = {"generator":"Gor-Desktop"}
		
		return html`
		<flow-caption-bar
			logo="/resources/images/Gor-logo-light-bg.png">Gor-Desktop</flow-caption-bar>
		<tab-content for="home">
			...
			<div id='Gor-resources'>

				<flow-expandable>
					<div slot="title" is="i18n-div" caption>GOR RESOURCES</div>
					<ul style="font-size: 12px;">
						
						</li>
						<li><flow-shell-link
							href="https://github.com/gordanet/"><flow-i18n>GitHub</flow-i18n></flow-shell-link></li>
						<li><flow-shell-link
							href="https://discord.com/invite/XrrPtjAkc8"><flow-i18n>Discord Chat</flow-i18n></flow-shell-link></li>
						<li><flow-link
			...
			<flow-expandable class="donation-addresses" no-info _icon="fal:donate">
				...
				<p is="i18n-p">
					if you wish to further the development of the gor ecosystem, we accept gor donations at the following addresses:
				</p>
				${
			...
			<flow-form-control icon="fal:copyright">
				<flow-i18n>Gor-Desktop &amp; Gor Copyright (c) 2024 Gor Developers<br/>
		...
		<tab-content for="settings">
			<flow-form-control icon="fal:database">
				...
				<p slot="info" is="i18n-p">
					Data Folder location is used for storage by all Gor-Desktop modules. 
					In default configuration this includes Gord blockchain data and GOR API database.
			...
			<flow-form-control icon="fal:tools">
				...
				<p slot="info" is="i18n-p">
					Advanced mode allows you to manually configure, interact with and monitor Gor-Desktop services.
				</p>
			</flow-form-control>
			<flow-form-control icon="fal:cog" class="advanced-tool">
				...
				<p slot="info" is="i18n-p">
					When enabled, Gor-Desktop runs itself and it's services hidden in the background
			...
			<flow-form-control icon="fal:cog" class="advanced-tool">
				...
				<p slot="info" is="i18n-p">When Autocompound enabled, Gor-Desktop compound UTXOs when there will be compoundable count of unspent transaction outputs (UTXOs).'Use latest Change address' will compund UTXOs using current/latest change address instead of first change address. </p>
			</flow-form-control>
			<flow-form-control icon="fal:cube" class="advanced-tool"
				...
				<p slot="info" is="i18n-p">
					The Enable Mining option starts / stops all configured Gorminer instances.
				</p>
			</flow-form-control>
			<!-- flow-form-control icon="fal:drafting-compass" class="advanced-tool">
				...
				<p slot="info" is="i18n-p">
					You can stream Gor-Desktop metrics to your own StatsD-compatible server.
				</p>
			</flow-form-control-->
			...
			<flow-form-control id="settings-script" icon="fal:tasks">
				...
				<p slot="info" is="i18n-p">
					Configuration editor allows you to customize Gor-Desktop environment.
					Gor-Desktop configuration is represented in JSON
			...
			<flow-form-control id="settings-templates" icon="fal:file-alt"
				...
				<p slot="info" is="i18n-p">
					Configuration templates allow you to load pre-made Gor-Desktop configurations.
				</p>
			</flow-form-control>
			<flow-form-control icon="fal:database">
				...
				<p slot="info" is="i18n-p">
					It will delete datadir (Data Folder) and restart gord node to re-sync
				</p>
			</flow-form-control>
			<flow-form-control icon="fal:database">
				...
				<p slot="info" is="i18n-p">
					Start the gord node without enabling the '--utxoindex' parameter until the synchronization is complete, re-enable this parameter. (Can be used to fix inaccurate balance display)
				</p>
			</flow-form-control>
			<div style="height:192px;"></div>
		</tab-content>
		<tab-content for="wallet" class="wallet" data-active-display="flex">
			<gor-wallet .walletMeta='${walletMeta}' hideNetwork hidefaucet 
				_hideQRScanner hideopenwalletlogo></gor-wallet>
		</tab-content>
		<tab-content for="console" data-active-display="flex" class="vertical-flex term">
			<flow-terminal id="gor-desktop-console" class="x-terminal" background="#000" foreground="#FFF"></flow-terminal>
		</tab-content>
...
//		this.checkForUpdates().then();
	...
	initRPC(){
		let rpc = new FlowRPC({bcastChannel:'gor-desktop'});
		this.rpc = rpc;
	...
	async initManager(){
		...
		manager.on("task-start", (daemon)=>{
			...
			this.refreshApps();
			const {wallet} = this;
			if(!wallet || daemon.task.type!='gord' || !this.rpcDisconnect){
				return
			}
		...
		manager.on("before-interrupt", ({daemon, interrupt})=>{
			...
			const {wallet} = this;
			if(!wallet || daemon.task.type!='gord')
				return
		...
		manager.on('sync-status', (data) => {
			//console.log("sync-status:data", data);
			let wallet = this.wallet || this.qS('gor-wallet');
	...
	async initWallet() {
		...
		let settings = await this.get_default_local_gord_settings();
	...
	async get_default_local_gord_settings() {
		
		let {config:daemons} = await this.get("get-modules-config");
		console.log("############### DAEMONS", daemons);
		let gord = Object.entries(daemons).map(([k,v]) => { 
			const { args } = v;
	...
	async initConsole() {
		let terminal = this.qS('#gor-desktop-console');
	...
	setTheme(theme){
		...
		if(this.caption)
			this.caption.logo = `/resources/images/gor-logo-${theme}-bg.png`
		...
		document.body.dispatchEvent(new CustomEvent("flow-theme-changed"));
		this.querySelector("gor-wallet")?.requestUpdate("theme", null)
	}
	initCaption(){
		...
		this.caption.logo = `/resources/images/gor-logo-${this.theme||"light"}-bg.png`;
	...
	async initTemplates() {
		...
		if(!this.tpl_template) {
			...
			let miner = Object.keys(config.modules).filter(v=>/^(gor|gpu)miner/.test(v));
			if(!miner.length)
	...
	async initSettings(){
		...
		if(statsdPrefixInput)
			this.statsdPrefix = statsdPrefixInput.value = config.statsdPrefix || "gor-desktop.$HOSTNAME";
	...
	initReleaseNotes(){
		...
		dialog.content = 
`#	Welcome to Gor-Desktop ${pkg.version}!
...
${changelogContent}`;
	...
	async initDaemons(daemons){
		...
		let {params} = this.getModuleArgs('gord:', daemons);
	...
	showApps(daemons) {
		...
		let entries = Object.values(appsMap).map((app) => {
			...
			if(!location && app.engines?.gor-desktop) {
			...
			return `${this.debugAppLinks?JSON.stringify(app)+"::"+location:""}
				<flow-window-link
					...
					width="${width}"
					height="${height}"
					icon="resources/images/gor-desktop-icon.png"
					resizable
					frame
	...
	async checkForUpdates(force = false) {
		...
		const url = 'https://gor-desktop.app/version.json';
		...
		if(!data) {
			console.log("missing version data in https://gor-desktop.app/version.json");
			return false;
		}
		...
		if(is_gt) {
			...
			let {btn} = await FlowDialog.show({
				title:i18n.t("Gor-Desktop Update"),
			...
			if(btn == 'ok') {
				require('nw.gui').Shell.openExternal('https://gor-desktop.app');

				let confirm = await FlowDialog.show({
					title:i18n.t("Gor-Desktop Update"),
					body:html`Please download the latest version from  <flow-link href="https://gor-desktop.app" target="_blank">https://gor-desktop.app</flow-link>
					<br/>&nbsp;<br/>
					Gor-Desktop will now shutdown`,
					btns:[{
						text:i18n.t('Cancel'),
						value:'cancel'
					},{
...
Gor-DesktopApp.define("gor-desktop-app")
